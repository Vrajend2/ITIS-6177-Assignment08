...
const options = {
  swaggerDefinition: {
    info: {
      ...
      description: "REST API Documentation Authored by Vignesh Babu Rajendran",
      version: "3.0.0"
    },
  },
  apis: ["server.js"],
...
app.get('/api/getAgent', async (req, res) => {
  try {
...
/**
 * @swagger
 * /api/patchCustomerByCustomerCode:
 *   patch:
...
/**
 * @swagger
 * /api/deleteCustomerByCustomerCode:
 *   delete:
...
app.delete("/api/deleteCustomerByCustomerCode", (req, res) => {
  const customerCode = req.query.customerCode;
  if (!customerCode) {
    res.status(400).json({ error: "Customer Code is required" });
    return;
